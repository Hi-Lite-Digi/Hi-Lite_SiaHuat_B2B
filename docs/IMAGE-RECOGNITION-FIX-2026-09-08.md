...
The first production browser check still returned the generic photo fallback. The initial implementation inferred recognition from the first description sentence, so an unreadable-brand statement could suppress a valid item type. Vision now returns a separate structured `imageCategory`, using only the current image and no earlier conversation history. Catalogue checks and reply wording remain separate. Diagnostics record only image-kind/raster categories and whether a type was recognised, never image pixels, filenames, keys or full descriptions. The revised local API checks passed for the knife with quantity 3 and an unreadable non-product fixture; regression tests cover an unreadable brand with a recognised type.
